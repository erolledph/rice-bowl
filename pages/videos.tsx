...
const VideosPage = () => {
	const [allVideos, setAllVideos] = useState<CookingVideo[]>([])
			...
			<Section>
				...
				<div className='mb-8'>
					...
					<p className='text-lg text-zinc-600 dark:text-zinc-400 font-medium mb-8'>
						Search millions of cooking videos from YouTube
					</p>

					{/* Search Info */}
					{displayedVideos.length > 0 && (
						<p className='text-sm text-zinc-500 dark:text-zinc-400 mb-4'>
							Showing results for: <span className='font-medium text-zinc-700 dark:text-zinc-300'>&quot;{currentSearchQuery}&quot;</span>
						</p>
					)}
				...
				{error && !loading && displayedVideos.length === 0 && (
					<div className='mt-12 text-center py-16'>
						...
						<p className='text-zinc-500 dark:text-zinc-500 max-w-md mx-auto mb-6'>
							{error || 'We are having trouble loading videos right now. Please try again.'}
						</p>
						<button
				...
				{!loading && displayedVideos.length > 0 && (
					<>
						...
						{!nextPageToken && displayedVideos.length > VIDEOS_PER_PAGE && (
							<div className='text-center py-8'>
								<p className='text-zinc-500 dark:text-zinc-500 text-sm font-medium'>
									✓ Loaded {displayedVideos.length} videos
								</p>
							</div>
						)}
					</>
				...
				{!loading && displayedVideos.length === 0 && !error && searchQuery && (
					<div className='mt-12 text-center py-16'>
